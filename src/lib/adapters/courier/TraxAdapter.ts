// src/lib/adapters/courier/TraxAdapter.ts
...
import {
  ...
  /**
   * Trax Courier Service Integration Adapter.
   * 
   * 🔧 REAL API Integration Ready:
   * - Uses credentials from Admin Settings
   * - Full error handling and validation
   * 
   * 📍 Trax API Endpoints (To be provided by Trax partner team):
   ...
   * - traxApiKey: API Key
   * - traxSecret: API Secret
   */
  export class TraxAdapter extends BaseCourierAdapter {
    private baseUrl: string;
  
    constructor(credentials: any) {
      super(credentials, "trax");
      this.baseUrl = credentials?.apiUrl || process.env.TRAX_API_URL || 'https://api.trax.com.pk/v1';
    }
    ...
    async generateAWB(data: ShipmentData): Promise<AWBResponse> {
      try {
        ...
        const payload = this.buildTraxPayload(data);
        ...
        const response = await this.makeRequest<{
          ...
          payload,
          {
            'X-API-Key': this.getCredential('traxApiKey') || '',
            'X-Secret': this.getCredential('traxSecret') || '',
          }
        );
        ...
        return {
          ...
          message: `Trax AWB generated successfully for ${data.orderNumber}`,
        };
      } catch (error) {
    ...
    async trackAWB(trackingId: string): Promise<TrackingResponse> {
      try {
        ...
        const mappedStatus = this.mapTraxStatus(response.status);
    ...
    async cancelAWB(trackingId: string, reason?: string): Promise<{ success: boolean; message: string }> {
      try {
        ...
        return {
          success: true,
          message: `Trax AWB ${trackingId} cancelled successfully.`,
        };
      } catch (error) {
        const err = this.handleError(error);
        return {
          success: false,
          message: err.message || 'Failed to cancel Trax AWB.',
        };
      }
    }
    ...
    async validateCredentials(): Promise<boolean> {
      try {
        this.requireCredential('traxApiKey', this.getCredential('traxApiKey'));
        this.requireCredential('traxSecret', this.getCredential('traxSecret'));
        return true;
      } catch (error) {
        throw new Error(`Trax credentials validation failed: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    private mapTraxStatus(rawStatus: string): string {
    ...
    private buildTraxPayload(data: ShipmentData): Record<string, unknown> {
      return {
