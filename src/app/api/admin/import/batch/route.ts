...
//     // 🖼️ 3. Extract Image Files from FormData
...
//     // 🔌 4. Connect to Payload
//     const payload = await getSafePayload();

//     // 📦 5. Pre-fetch Categories & Brands (for mapping)
...
//       const parentImages = productMeta.parentCompressedImages || [];
...
//                 );
//                 if (!found) throw new Error(`Category "${cleanName}" not found.`);
//                 return found.id;
//               })
//               .filter(Boolean)
//           : [];

//         // --- Get Adapters for this product (respects storageProvider & primaryProvider) ---
...
//         // Map parent images
//         const parentImageNames = parentImages || [];
//         const parentImageFiles = parentImageNames.map((name: string) => {
//           // Find the file in formData by matching name (or we can use index)
//           // Better: we have the order of images in the metadata.
//           // We'll just iterate and pick them in order.
//           return null; // Placeholder, will be handled below
//         });

//         // Since we have the list of compressed image names in the metadata,
//         // we need to match them with the uploaded files.
//         // The frontend sends images as image_0, image_1, etc.
//         // The metadata stores the original filenames to match.
//         // For simplicity, we'll assume the images are in the same order as the metadata.
//         // We'll process all images for the product sequentially.

//         // --- Variants Processing ---
//         const allVariantRows = [...variantMetas];
//         // If parent has variant data, add it as a variant
...
//               try {
//                 // Upload to ALL adapters (dual upload)
...
//                 // Build image object with all URLs
...
//                 // Add placeholder to avoid breaking the variant
//                 uploadedImages.push({
//                   _key: generateKey(),
//                   url: '/placeholder.png',
//                   imgbbUrl: null,
...
//     // 💾 7. Insert Products into Payload
//     let successCount = 0;
//     let failCount = 0;
