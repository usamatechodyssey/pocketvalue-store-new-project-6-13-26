...
//   // Track initial payload (server-side state) to prevent duplicate fetches
//   const initialPayloadRef = useRef<any>(null);
//   const hasMountedRef = useRef(false);

//   // Compute initial payload once on mount
//   useEffect(() => {
...
//   // Main fetch effect – skip if still initial state
//   useEffect(() => {
//     // If we haven't computed initial payload yet (first render), skip
...
//     // Compare with initial payload to skip if unchanged
//     const payloadString = JSON.stringify(payload);
//     const initialString = JSON.stringify(initialPayloadRef.current);

//     // If the current payload matches the initial server state and it's the first load, skip fetch
//     if (payloadString === initialString && hasMountedRef.current) {
//       // Still allow subsequent fetches after user interaction, but if it's exactly the initial state, skip
//       // But we need to allow fetches when page changes to 1 after filters are cleared? We'll rely on the check below.
//       // Actually, if it's exactly the initial state, we should skip only if products are already set (they are)
//       // and we haven't made any changes. So we skip.
//       // However, if the user manually resets filters, the payload will match initial, but we want to keep products as is.
//       // So skipping is fine.
//       return;
//     }

//     // Additionally, prevent duplicate fetches with same payload (after initial)
...
//   // Track previous payload to prevent duplicate consecutive fetches
...
//   // ✅ UPGRADED: Determine canonical URL (without filters) for SEO (#5)
...
//   return (
//     <>
//       {/* ✅ UPGRADED: Canonical link for filtered views (#5) */}
//       {hasActiveFilters && (
...
export default function ProductListingClient({
  ...
  // ✅ ENTERPRISE FIX: Preserve attribute name case (e.g., "Color", "Size")
  // Handles both { _id, values } from aggregation and { name, value } from type
  const uniqueAttributes = useMemo(() => {
    const attrs: Record<string, Set<string>> = {};
    if (filterData?.attributes) {
      filterData.attributes.forEach((attr: any) => {
        // Support both shapes: { _id, values } or { name, value }
