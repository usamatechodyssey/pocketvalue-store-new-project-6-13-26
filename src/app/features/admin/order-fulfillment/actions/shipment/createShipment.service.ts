// // 📂 src/features/admin/order-fulfillment/actions/shipment/createShipment.service.ts (FULLY HARDENED FOR PRODUCTION)
...
//  * - Dual Variant Resolution: Matches both _key and id properties
...
//     // 🛡️ 6. Verify Items Exist in Order (Dual Variant Resolution)
//     for (const item of items) {
//       // ✅ CRITICAL FIX: Dual Variant Key Matcher prevents "Product not found" errors on Payload subdocuments
//       const orderItem = order.products.find((p: any) => {
//         const isProductMatch = p.productId === item.productId;
//         const isVariantMatch = 
//           (p.variant?._key && p.variant._key === item.variantKey) ||
//           (p.variant?.id && p.variant.id === item.variantKey) ||
//           (!item.variantKey); // Fallback for single default variant
...
//     // 🛡️ 7. Deduct Stock (Payload DB) with Compensating Rollback
...
//     } catch (awbError: any) {
//       // Rollback stock deductions if AWB generation throws HTTP/API exceptions
//       for (const prev of deductionResults) {
//         await restoreVariantStock(prev.productId, prev.variantKey, prev.quantity);
//       }
//       return {
...
//       // Rollback stock deductions if courier API returns error response
//       for (const prev of deductionResults) {
//         await restoreVariantStock(prev.productId, prev.variantKey, prev.quantity);
//       }
//       return {
...
//   }
// }
// 📂 src/app/features/admin/order-fulfillment/actions/shipment/createShipment.service.ts
...
// ================================================================
// 🚀 SERVICE: CREATE SINGLE SHIPMENT (Cache Synced)
// ================================================================
...
export async function createShipment(
  ...
  try {
    ...
    try {
      ...
      // Rollback stock deductions if AWB generation throws exceptions
      for (const prev of deductionResults) {
        await restoreVariantStock(prev.productId, prev.variantKey, prev.quantity);
      }
      return {
    ...
    // ✅ 🆕 FIX: Purge Redis caches across 3 dashboards upon successful shipment creation!
    try {
      ...
      await pipeline.exec();
      console.log(`⚡ Shipment Creation Sync: Purged Redis caches across Executive, Operational, and Geospatial views.`);
