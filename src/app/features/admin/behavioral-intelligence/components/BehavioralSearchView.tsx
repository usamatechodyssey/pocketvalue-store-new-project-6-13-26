// 📂 src/app/features/admin/behavioral-intelligence/components/BehavioralSearchView.tsx

"use client";

import React, { useState, useEffect } from "react";
import { useSearchParams } from "next/navigation";
import { getBehavioralSearch } from "../actions/getBehavioralSearch";
import {
  Search,
  MousePointerClick,
  TrendingUp,
...
// 🚀 MAIN COMPONENT
// ================================================================
export default function BehavioralSearchView({ range }: { range: { from: Date; to: Date } }) {
  ...
  useEffect(() => {
    setLoading(true);
    getBehavioralSearch(range, currentPage)
  ...
  if (!data || data.summary.totalSearches === 0) {
    return (
      <div className="p-8 text-center bg-zinc-50/50 dark:bg-zinc-900/30 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl flex flex-col items-center justify-center min-h-75 w-full">
        ...
        <p className="text-xs font-mono text-zinc-500 dark:text-zinc-400 max-w-sm mt-1">
          No search queries or click telemetry recorded in the selected audit range.
        </p>
      </div>
    );
  }
  ...
  const trending = data.trendingTerms;
  ...
  return (
    <div className="space-y-6 w-full min-w-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="p-3.5 bg-zinc-50/80 dark:bg-zinc-900/60 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl shadow-2xs flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-widest">Total Searches</span>
          </div>
          <p className="text-lg sm:text-xl font-black text-zinc-900 dark:text-zinc-50 font-mono tracking-tight leading-none">{summary.totalSearches.toLocaleString('en-PK')}</p>
        </div>
        ...
        <div className="p-3.5 bg-zinc-50/80 dark:bg-zinc-900/60 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl shadow-2xs flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-widest">Zero Results</span>
          </div>
          <p className="text-lg sm:text-xl font-black text-zinc-900 dark:text-zinc-50 font-mono tracking-tight leading-none">{summary.zeroResultSearches.toLocaleString('en-PK')}</p>
        </div>
      </div>

      {/* ================================================================ */}
      {/* 🔥 2. TRENDING SEARCHES (Horizontal Bar) */}
      {/* ================================================================ */}
      {trending.length > 0 && (
        <div className="bg-zinc-50/60 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl p-6 shadow-2xs">
          <h3 className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 uppercase tracking-widest mb-6 flex items-center gap-2 border-b border-zinc-200/80 dark:border-zinc-800/80 pb-3">
            <BarChart3 size={16} className="text-brand-primary" /> TRENDING SEARCH TERMS
          </h3>
            ...
            {trending.map((item: any, idx: number) => {
              ...
              return (
                <div key={item.term} className="flex items-center gap-4">
                  ...
                  <span className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 flex-1 truncate" title={item.term}>
                    {item.term}
                  </span>
                  <div className="flex items-center gap-3 w-56 shrink-0">
                    ...
                    <span className="text-[9px] font-mono font-bold text-zinc-400 w-10 text-right">
                      {Math.round(percentage)}%
                    </span>
                    <span className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-50 w-10 text-right">
                      {item.count.toLocaleString('en-PK')}
                    </span>
                  </div>
                </div>
              );
      ...
      <div className="bg-zinc-50/60 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl p-6 shadow-2xs">
        <div className="flex items-center justify-between border-b border-zinc-200/80 dark:border-zinc-800/80 pb-3 mb-5">
          <span className="text-[10px] font-mono font-bold text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-3 py-1 rounded-full uppercase tracking-widest flex items-center gap-1.5">
            <TrendingUp size={12} className="text-brand-primary" /> DAILY SEARCH VOLUME TRENDS
          </span>
        </div>
        <div className="w-full h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="searches" stroke="#8b5cf6" name="Searches" strokeWidth={3} dot={false} />
      ...
      {/* ================================================================ */}
      {/* 🔥 4. SEARCH TERMS DATA TABLE (With Scroll Guard) */}
      {/* ================================================================ */}
          ...
          <table className="w-full min-w-175 border-collapse text-left text-xs relative" role="table">
            <thead className="sticky top-0 z-10 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border-b border-zinc-200/80 dark:border-zinc-800/80">
              <tr className="text-[10px] uppercase font-mono font-bold text-zinc-400 dark:text-zinc-500 tracking-wider">
                <th className="py-3 px-4 whitespace-nowrap">Search Term</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">Searches</th>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60 bg-white/40 dark:bg-zinc-950/40 font-mono">
              {searchTerms.map((item: any) => (
                <tr key={item.term} className="hover:bg-zinc-100/60 dark:hover:bg-zinc-900/60 transition-colors">
                  <td className="py-3 px-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-zinc-900 dark:text-zinc-100 truncate max-w-40" title={item.term}>
                        {item.term}
                      </span>
                      <button
                        onClick={() => copyToClipboard(item.term, "Search Term")}
                  ...
                  <td className="py-3 px-4 text-center text-xs font-bold text-zinc-900 dark:text-zinc-100 whitespace-nowrap">
                    {item.searches.toLocaleString('en-PK')}
                  </td>
                  ...
                  <td className="py-3 px-4 text-center whitespace-nowrap">
                    {item.zeroResults ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-red-500/10 text-red-500 border border-red-500/20 text-[9px] font-bold uppercase">
                        <AlertCircle size={10} /> Zero Results
                      </span>
                    ) : (
