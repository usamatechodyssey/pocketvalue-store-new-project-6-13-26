...
export default function BehavioralDemandView({ range }: { range: { from: Date; to: Date } }) {
  ...
  // ✅ Reactive state for dynamic dropdown filter
  const [statusFilter, setStatusFilter] = useState<string>(urlStatusFilter);

  useEffect(() => {
    setLoading(true);
    // ✅ Ingests active status filter dynamically into getBehavioralDemand
    getBehavioralDemand(range, currentPage, 25, statusFilter as any)
      .then((res) => setData(res))
      .catch(() => setData(null))
      .finally(() => setLoading(false));
  }, [range, currentPage, statusFilter]);

  const handleStatusChange = (newStatus: string) => {
    setStatusFilter(newStatus);
    const params = new URLSearchParams(searchParams.toString());
    params.set("status", newStatus);
    params.set("page", "1"); // Reset to page 1 on filter trigger
...
{topVariants.map((item: any, idx: number) => {
  ...
  const percentage = (item.requests / maxRequests) * 100;
  return (
    // ✅ FIX: Combined key, name, and index to guarantee absolute uniqueness
    <div key={`${item.variantKey}_${item.variantName}_${idx}`} className="flex items-center gap-4">
      <span className="font-mono text-xs font-black text-zinc-400 w-6">#{String(idx + 1).padStart(2, '0')}</span>
      <span className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 flex-1 truncate" title={item.variantName}>
        {item.variantName}
        <span className="text-[8px] text-zinc-400 dark:text-zinc-500 ml-2 font-mono">({item.variantKey})</span>
      ...
      {/* ================================================================ */}
      {/* 🔥 5. REQUESTS TABLE WITH STATUS DROPDOWN FILTER */}
      {/* ================================================================ */}
      <div className="bg-zinc-50/60 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xs flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-zinc-200/80 dark:border-zinc-800/80 bg-white/50 dark:bg-zinc-950/50 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 uppercase tracking-tight">Demand Requests Pipeline</h3>
            <span className="text-[10px] font-mono font-bold text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-2 py-0.5 rounded-full">
              {data.totalDocs.toLocaleString('en-PK')}
            </span>
          </div>

          {/* ✅ FIX: Dynamic Status Dropdown Filter integrated beautifully inside table header */}
          <div className="flex items-center gap-2 print:hidden">
            <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-wider">Filter Status:</span>
            <select
              value={statusFilter}
              onChange={(e) => handleStatusChange(e.target.value)}
              className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-1.5 text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 focus:outline-none focus:border-brand-primary transition-all shadow-2xs shrink-0"
            >
