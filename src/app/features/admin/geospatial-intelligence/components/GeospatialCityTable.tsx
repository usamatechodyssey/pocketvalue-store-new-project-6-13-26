// 📂 src/app/features/admin/geospatial-intelligence/components/GeospatialCityTable.tsx

"use client";

import React, { useState, useMemo } from 'react';
import { GeospatialCityData } from '../actions/getGeospatialIntelligence';
import { TrendingUp, TrendingDown, Minus, Zap, MapPin, Search } from 'lucide-react';

interface Props {
  cities: GeospatialCityData[];
}

export default function GeospatialCityTable({ cities }: Props) {
  const [searchTerm, setSearchTerm] = useState("");

  // ✅ Real-time Filtered Cities List
  const filteredCities = useMemo(() => {
    if (!cities || cities.length === 0) return [];
    if (!searchTerm.trim()) return cities;
    const term = searchTerm.toLowerCase();
    return cities.filter(
      (c) => c.city.toLowerCase().includes(term) || c.province.toLowerCase().includes(term)
    );
  }, [cities, searchTerm]);

  // Max Revenue for progress bar scaling
  const maxRevenue = useMemo(() => {
    if (!cities || cities.length === 0) return 1;
    return Math.max(...cities.map((c) => c.revenue)) || 1;
  }, [cities]);

  // Helper: RTO Color Coding
  const getRtoColor = (rate: number) => {
    if (rate <= 5) return "text-emerald-500 font-mono font-bold";
    if (rate <= 15) return "text-amber-500 font-mono font-bold";
    return "text-red-500 font-mono font-bold";
  };

  // ✅ Empty State
  if (!cities || cities.length === 0) {
    return (
      <div
        className="p-8 text-center bg-zinc-50/50 dark:bg-zinc-900/30 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl flex flex-col items-center justify-center min-h-50"
        role="status"
        aria-label="No city data available"
      >
        <div className="flex flex-col items-center gap-3 max-w-xs mx-auto">
          <div className="p-3 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 rounded-2xl">
            <MapPin size={28} />
          </div>
          <h3 className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 uppercase tracking-tight">
            No City Data Available
          </h3>
          <p className="text-[10px] font-mono text-zinc-500 dark:text-zinc-400">
            No orders with verified city data were found in the selected audit period.
          </p>
        </div>
      </div>
    );
  }

  return (
    <div
      className="space-y-4 w-full min-w-0 animate-in fade-in duration-300"
      role="table"
      aria-label="Geospatial city performance table"
    >
      {/* SEARCH FILTER TOOLBAR */}
      <div className="p-3.5 bg-zinc-50/80 dark:bg-zinc-900/60 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl flex items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400" />
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search city or province name..."
            className="w-full pl-9 pr-4 py-1.5 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-mono text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-hidden focus:border-brand-primary transition-all"
          />
        </div>
        <span className="text-[10px] font-mono font-bold text-zinc-500 dark:text-zinc-400 hidden sm:inline">
          SHOWING {filteredCities.length} OF {cities.length} CITIES
        </span>
      </div>

      {/* TABLE CONTAINER (With Scroll Guard & Zero Squeezing) */}
      <div className="bg-zinc-50/60 dark:bg-zinc-900/40 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xs">
        <div className="overflow-x-auto max-h-112.5 custom-scrollbar">
          <table className="w-full min-w-175 border-collapse text-left text-xs relative">
            <thead className="sticky top-0 z-10 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md border-b border-zinc-200/80 dark:border-zinc-800/80">
              <tr className="text-[10px] uppercase font-mono font-bold text-zinc-400 dark:text-zinc-500 tracking-wider">
                <th className="py-3 px-4 whitespace-nowrap">City</th>
                <th className="py-3 px-4 whitespace-nowrap">Province</th>
                <th className="py-3 px-4 text-right whitespace-nowrap">Revenue</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">Orders</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">AOV</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">RTO %</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">Trend</th>
                <th className="py-3 px-4 text-center whitespace-nowrap">Potential</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60 bg-white/40 dark:bg-zinc-950/40">
              {filteredCities.map((c) => {
                const revenuePercent = maxRevenue > 0 ? (c.revenue / maxRevenue) * 100 : 0;

                return (
                  <tr
                    key={c.city}
                    className="hover:bg-zinc-100/60 dark:hover:bg-zinc-900/60 transition-colors"
                  >
                    <td className="py-3 px-4 font-bold text-zinc-900 dark:text-zinc-100 whitespace-nowrap font-mono">
                      {c.city}
                    </td>
                    <td className="py-3 px-4 text-zinc-500 dark:text-zinc-400 font-mono text-[10px] uppercase whitespace-nowrap">
                      {c.province}
                    </td>

                    {/* Revenue with progress bar */}
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-2">
                        <span className="font-mono font-bold text-brand-primary text-xs">
                          Rs. {(c.revenue || 0).toLocaleString('en-PK')}
                        </span>
                        <div className="w-12 h-1.5 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden shrink-0">
                          <div
                            className="h-full bg-brand-primary transition-all duration-500"
                            style={{ width: `${Math.min(revenuePercent, 100)}%` }}
                          />
                        </div>
                      </div>
                    </td>

                    <td className="py-3 px-4 text-center font-mono font-semibold text-zinc-700 dark:text-zinc-300 whitespace-nowrap">
                      {(c.orders || 0).toLocaleString('en-PK')}
                    </td>
                    <td className="py-3 px-4 text-center font-mono font-semibold text-zinc-700 dark:text-zinc-300 whitespace-nowrap">
                      Rs. {(c.aov || 0).toLocaleString('en-PK')}
                    </td>

                    {/* RTO Rate */}
                    <td className={`py-3 px-4 text-center whitespace-nowrap ${getRtoColor(c.rtoRate)}`}>
                      {c.rtoRate}%
                    </td>

                    {/* Trend Icon */}
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      {c.trend === 'STAR' && (
                        <TrendingUp size={14} className="text-emerald-500 mx-auto" />
                      )}
                      {c.trend === 'FALLING' && (
                        <TrendingDown size={14} className="text-red-500 mx-auto" />
                      )}
                      {c.trend === 'STABLE' && (
                        <Minus size={14} className="text-zinc-400 mx-auto" />
                      )}
                    </td>

                    {/* High Potential Flag */}
                    <td className="py-3 px-4 text-center whitespace-nowrap">
                      {c.isHighPotential ? (
                        <Zap size={14} className="text-amber-500 mx-auto animate-pulse" />
                      ) : (
                        <span className="text-zinc-400 dark:text-zinc-600">—</span>
                      )}
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>

        {/* Footer */}
        <div className="p-3 border-t border-zinc-200/80 dark:border-zinc-800/80 bg-white/50 dark:bg-zinc-950/50 flex justify-between items-center text-[9px] font-mono text-zinc-400 dark:text-zinc-500 flex-wrap gap-2">
          <span>📍 {filteredCities.length} Cities Analyzed</span>
          <span>📊 Revenue Bar = % of Max City Revenue</span>
          <span>🟢 RTO ≤5% • 🟡 5-15% • 🔴 &gt;15%</span>
        </div>
      </div>
    </div>
  );
}