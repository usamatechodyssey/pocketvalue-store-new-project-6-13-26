...
//   );
// }
// 📂 src/app/features/admin/reports/components/ReportTableSection.tsx (MONTHLY DATE FORMATTER HARDENED)
...
const formatValue = (value: any, formatType: ReportColumnFormat, isMounted: boolean): string => {
  ...
  switch (formatType) {
    ...
    case "date": {
      ...
      // ✅ FIX: Monthly string "yyyy-MM" (e.g. 2026-08) formatted as "Aug 2026"
      if (/^\d{4}-\d{2}$/.test(strVal)) {
        const [y, m] = strVal.split("-");
        const d = new Date(Number(y), Number(m) - 1, 1);
        return d.toLocaleDateString("en-PK", { year: "numeric", month: "short" });
      }
      // SSR Hydration Safeguard: Render ISO raw date until client mounts safely
      if (!isMounted) return strVal.split("T")[0];
      return new Date(value).toLocaleDateString("en-PK", {
        year: "numeric",
        month: "short",
        day: "numeric",
...
// ================================================================
// 🎨 SEMANTIC CELL COLORING HELPER
// ================================================================
const getSemanticCellColor = (key: string, isTotalRow: boolean = false): string => {
  const k = key.toLowerCase();

  // Pure Profit / Target Profit / Gross Profit -> Emerald Green
  if (
    k.includes("netprofit") || 
    k.includes("pureprofit") || 
    k.includes("targetprofit") || 
    k.includes("grossprofit") || 
    k.includes("profit")
  ) {
    return "text-emerald-600 dark:text-emerald-400 font-bold";
  }

  // Discounts & Losses -> Rose / Red
  if (k.includes("discount") || k.includes("loss") || k.includes("refund")) {
    return "text-rose-600 dark:text-rose-400 font-medium";
  }

  // Revenue & Sales -> Pure High-Contrast White / Zinc
  if (k.includes("sales") || k.includes("revenue")) {
          ...
          <thead>
            <tr className="bg-zinc-50/50 dark:bg-zinc-900/30 border-b border-zinc-200 dark:border-zinc-800">
              {visibleColumns.map((col) => (
                <th
                  key={col.key}
                  className={`py-3 px-2.5 whitespace-nowrap text-[10px] uppercase text-zinc-400 dark:text-zinc-500 font-bold tracking-wider select-none ${
                    col.sortable !== false ? "cursor-pointer hover:text-brand-primary transition-colors" : ""
                  } ${getAlignClass(col.align)}`}
                  onClick={() => col.sortable !== false && handleSort(col.key)}
                  style={col.width ? { minWidth: col.width } : undefined}
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-850">
            {sortedData.map((row, rowIndex) => (
              ...
              >
                {visibleColumns.map((col) => {
                  const isNumeric = col.format === "currency" || col.format === "percentage" || col.format === "number";
                  const colorClass = getSemanticCellColor(col.key, false);

                  return (
                    <td
                      key={col.key}
                      className={`py-2.5 px-2.5 whitespace-nowrap ${colorClass} ${
                        isNumeric ? "font-mono tracking-tight" : ""
                      } ${getAlignClass(col.align)}`}
                    >
                      {formatValue(row[col.key], col.format, mounted)}
                ...
                {visibleColumns.map((col, index) => {
                  if (index === 0) {
                    return (
                      <td
                        key={col.key}
                        className="py-3 px-2.5 whitespace-nowrap text-xs font-bold text-zinc-900 dark:text-zinc-100"
                      >
                        Totals
                      </td>
                    );
                  ...
                  const totalValue = totals[col.key];
                  const isNumeric = col.format === "currency" || col.format === "percentage" || col.format === "number";
                  const colorClass = getSemanticCellColor(col.key, true);

                  if (totalValue !== undefined && totalValue !== null) {
                    return (
                      <td
                        key={col.key}
                        className={`py-3 px-2.5 whitespace-nowrap text-xs ${colorClass} ${
                          isNumeric ? "font-mono tracking-tight" : ""
                        } ${getAlignClass(col.align)}`}
                      >
                        {formatValue(totalValue, col.format, mounted)}
                  ...
                  return (
                    <td
                      key={col.key}
                      className={`py-3 px-2.5 whitespace-nowrap ${getAlignClass(col.align)}`}
                    >
                      —
                    </td>
                  );
