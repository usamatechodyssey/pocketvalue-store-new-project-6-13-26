...
//   }
// }
// 📂 src/app/features/admin/reports/actions/getReturnRequestsSummaryData.ts
...
import {
  buildReturnRequestsSummaryMatrix,
  UnifiedReturnRequestSummaryRow,
...
// ================================================================
// 🚀 MAIN SERVER ACTION (Delegated to Central Engine)
// ================================================================
export async function getReturnRequestsSummaryData(
  range: { startDate: Date; endDate: Date }
): Promise<{ success: boolean; data?: ReturnRequestsSummaryResponse; error?: string }> {
  const cacheKey = `analytics_return_summary_v5:${format(range.startDate, "yyyy-MM-dd")}_${format(range.endDate, "yyyy-MM-dd")}`;
  ...
  try {
    ...
    const parsed = safeParse<ReturnRequestsSummaryResponse>(cachedData as string | null);
    if (parsed) {
      console.log(`⚡ Redis Cache Hit: Return Requests Summary (Central Engine)`);
      return { success: true, data: parsed };
    }
    ...
    const engineResult = await buildReturnRequestsSummaryMatrix({
      startDate: start,
      endDate: end,
    });

    const data: ReturnStatusSummaryRow[] = engineResult.map((item: UnifiedReturnRequestSummaryRow) => ({
      status: item.status,
      count: item.count,
    }));
