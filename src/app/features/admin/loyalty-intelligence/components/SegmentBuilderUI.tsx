...
export default function SegmentBuilderUI({ initialSegmentId }: SegmentBuilderUIProps) {
  ...
  // ================================================================
  // 📋 LOAD SAVED SEGMENTS LIST (Shared Helper)
  // ================================================================
  const fetchSavedSegmentsList = useCallback(async () => {
    setIsListLoading(true);
    try {
      const response = await fetch("/api/admin/segments");
      if (!response.ok) throw new Error("Failed to load segments.");
      ...
      console.warn("⚠️ Failed to load saved segments:", error);
    } finally {
      setIsListLoading(false);
    }
  }, []);

  // ✅ FIX 1: Auto-fetch saved segments count on initial component mount!
  useEffect(() => {
    fetchSavedSegmentsList();
    if (initialSegmentId) {
      loadSegment(initialSegmentId);
    }
  }, [initialSegmentId, fetchSavedSegmentsList]);
  ...
  // ================================================================
  // 💾 SAVE SEGMENT (With Auto-Refetch)
  // ================================================================
  const handleSave = async () => {
    if (!name.trim()) {
      toastError("Please enter a segment name.");
    ...
    try {
      ...
      // ✅ FIX 2: Refetch saved segments list immediately so "Load Saved (X)" count updates instantly!
      await fetchSavedSegmentsList();
  ...
  // ================================================================
  // 🗑️ DELETE SEGMENT (With Auto-Refetch)
  // ================================================================
  const handleDelete = async () => {
    if (!segmentId) return;
    if (!confirm("Are you sure you want to delete this segment?")) return;
    ...
    try {
      ...
      // ✅ FIX 3: Refetch list after delete
      await fetchSavedSegmentsList();
      router.replace("/admin/segment-builder");
    } catch (error) {
      toastError("Failed to delete segment.");
