...
//   }
// }
// 📂 src/app/features/admin/loyalty-intelligence/actions/saveSegment.ts
...
// ✅ IMPORT GET SEGMENT QUERY FOR INITIAL LASTRUNCOUNT SNAPSHOT
import { getSegmentQuery } from "./getSegmentQuery";
...
// ================================================================
// 🚀 MAIN ACTION: Save Segment Definition (With Live Initial Count)
// ================================================================
export async function saveSegment(data: {
  id?: string;
  name: string;
  ...
  try {
    ...
    // ✅ FIX: Calculate live matching count for initial snapshot
    let initialMatchCount = 0;
    try {
      const matchResult = await getSegmentQuery(filters, 1, 1);
      initialMatchCount = matchResult.totalDocs || 0;
    } catch (countErr) {
      console.warn("⚠️ Failed to calculate initial lastRunCount on save:", countErr);
    }
    ...
    if (id) {
      ...
      existing.description = description || "";
      existing.filters = filters;
      existing.lastRunCount = initialMatchCount; // ✅ Stamped!
    ...
    const newSegment = new SegmentDefinition({
      ...
      description: description || "",
      filters,
      lastRunCount: initialMatchCount, // ✅ Stamped!
      lastRunAt: new Date(),
      isActive: isActive !== undefined ? isActive : true,
      createdBy: createdByObjectId,
    });
...
export async function listSegments(): Promise<{
  ...
  try {
    ...
    const _userModelSafety = User.modelName;

    const segments = await SegmentDefinition.find({ isActive: true })
      .sort({ createdAt: -1 })
      .populate("createdBy", "name email")
      .lean();
