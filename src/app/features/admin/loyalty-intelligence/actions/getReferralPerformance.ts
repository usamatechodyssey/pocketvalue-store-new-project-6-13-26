...
//   }
// }
// 📂 src/app/features/admin/loyalty-intelligence/actions/getReferralPerformance.ts
...
// ================================================================
export async function getReferralPerformance(range?: {
  startDate: Date;
  endDate: Date;
}): Promise<{
  success: boolean;
  ...
  const cacheKey = `analytics_referral_performance_v4:${fromStr}_${toStr}`;
  ...
  try {
    ...
    try {
      ...
      if (cachedData) {
        console.log(`⚡ Redis Cache Hit: Referral Performance (${fromStr} to ${toStr})`);
        const parsed = safeParse<ReferralPerformanceStats>(cachedData as string | null);
        if (parsed) {
          return { success: true, data: parsed };
        }
    ...
    // ================================================================
    // 🔥 1. AGGREGATE REFERRALS (WITH TIMEFRAME BOUNDARIES)
    // ================================================================
    const referralAggregation = await Referral.aggregate([
      // ✅ Match referrals created within selected date range
      {
        $match: {
    ...
    // 7. Write to Redis Cache
    try {
      await redis.set(cacheKey, safeStringify(compiledStats), {
        ex: 300,
      });
      console.log(`✅ Referral Performance cached (${fromStr} to ${toStr}).`);
    } catch (redisWriteError) {
      console.error("⚠️ Failed to write performance cache:", redisWriteError);
    }
