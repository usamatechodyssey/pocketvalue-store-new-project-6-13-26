...
//               <span className="w-2.5 h-2.5 rounded-full bg-brand-primary shadow-[0_0_8px_#f97316]" />
...
// ================================================================
// 🎨 KPI-MATCHING TREND BADGE COMPONENT
// ================================================================
const TrendBadge = ({ change, trend }: { change: number; trend: "UP" | "DOWN" | "STABLE" }) => {
  if (trend === "UP") {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-mono font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
  ...
  if (trend === "DOWN") {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-mono font-bold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20">
  ...
  return (
    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-mono font-bold bg-zinc-500/10 text-zinc-500 dark:text-zinc-400 border border-zinc-500/20">
...
// ================================================================
// 🔨 HELPER: Transform & Scale data for chart
// ================================================================
const transformChartData = (data: OperationalComparisonResponse) => {
  const metrics = data.metrics;
...
// ================================================================
// 🎨 SOLID CYBER-HUD TOOLTIP (KPICard-Matched Badges)
// ================================================================
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length > 0) {
    const item = payload[0]?.payload;
    ...
    return (
      <div
        className="bg-zinc-950/95 dark:bg-black/95 border border-zinc-800/80 p-4 rounded-2xl shadow-2xl min-w-64 backdrop-blur-md z-50 font-mono"
        role="tooltip"
        aria-label={`Comparison data for ${label}`}
      >
        <p className="text-[10px] font-bold uppercase text-zinc-400 mb-3 border-b border-zinc-800/80 pb-2 tracking-wider">
          {label}
        </p>
        <div className="space-y-2.5 text-xs">
          <div className="flex justify-between items-center gap-4">
            <span className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-brand-primary shadow-[0_0_8px_#f97316]" />
              <span className="text-zinc-400 uppercase text-[9px] font-bold">Current</span>
            </span>
          ...
          <div className="flex justify-between items-center gap-4">
            <span className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-zinc-500 border border-dashed" />
              <span className="text-zinc-400 uppercase text-[9px] font-bold">Previous</span>
            </span>
            <span className="font-bold text-zinc-400 font-mono">
              {formatValue(rawPrevious)}
            </span>
          </div>
          <div className="border-t border-zinc-800/80 pt-2.5 mt-1 flex justify-between items-center">
            <span className="font-bold text-zinc-500 uppercase text-[9px] tracking-widest">Growth Delta</span>
...
export default function OperationalComparisonChart({
  ...
  if (!mounted) {
    return (
      <div
        className="w-full h-95 bg-zinc-100 dark:bg-zinc-900/40 rounded-2xl animate-pulse flex items-center justify-center"
        role="status"
        aria-label="Loading operational comparison chart"
      >
        <Loader2 size={28} className="text-zinc-400 animate-spin" />
  ...
  if (!data || !data.metrics) {
    return (
      <div
        className="flex flex-col items-center justify-center h-95 bg-zinc-50/50 dark:bg-zinc-900/30 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 p-6 text-center"
        role="status"
        aria-label="No comparison data available"
      >
        <div className="p-3 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 rounded-2xl mb-3">
