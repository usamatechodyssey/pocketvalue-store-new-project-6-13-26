...
export const ContactItem = ({
  ...
  <li className="flex items-start gap-3 group">
    <Icon
      className="mt-1 h-4 w-4 shrink-0 text-brand-primary group-hover:scale-110 transition-transform"
      aria-hidden="true"
    />
    {href ? (
      <a
  ...
  return (
    <div className="border-b border-gray-800/50">
      <button
        ...
        className="w-full flex justify-between items-center py-5 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:ring-offset-2 focus:ring-offset-transparent rounded-lg"
      >
        <h3 className="font-bold text-gray-100 text-base uppercase tracking-wider">
          {title}
        </h3>
        <FiChevronDown
          className={`w-5 h-5 text-brand-primary transition-transform duration-500 ${
            isOpen ? "rotate-180" : ""
          }`}
          aria-hidden="true"
        />
