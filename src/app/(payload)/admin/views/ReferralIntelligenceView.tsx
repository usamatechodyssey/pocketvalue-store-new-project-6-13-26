...
//     getInactiveCustomers({ page: 1, limit: 1 }), // Returns raw data object
//     listSegments(),
//   ]);
...
//                 <BrainCircuit size={32} className="text-brand-primary shrink-0 animate-pulse" />
...
//               href={exportUrl}
//               target="_blank"
//               className="inline-flex items-center gap-1.5 px-5 py-3 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 rounded-2xl text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 transition-all border border-zinc-200 dark:border-zinc-800 uppercase tracking-wider no-underline hover:no-underline cursor-pointer shadow-lg active:scale-95"
//               title="Export referral logs to CSV"
//             >
//               <Download size={14} />
//               Export Logs CSV
//             </a>
...
//         {/* ================================================================ */}
//         {/* SECTION 5 & 6: INACTIVE CUSTOMERS + SEGMENT BUILDER (UNIFIED SHIELDS!) */}
//         {/* ================================================================ */}
//         <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
//           {/* ✅ FIX: Wrapped InactiveCustomersWidget inside standard high-density rounded card */}
//           <div className="bg-white dark:bg-zinc-950 p-6 sm:p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800/80 shadow-2xl relative overflow-hidden">
...
//           {/* ✅ FIX: Wrapped SegmentBuilderWidget inside standard high-density rounded card */}
//           <div className="bg-white dark:bg-zinc-950 p-6 sm:p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800/80 shadow-2xl relative overflow-hidden">
//             <div className="absolute top-0 right-0 w-80 h-80 bg-brand-primary/10 rounded-full blur-3xl pointer-events-none hidden dark:block" />
...
//             <span>📊 Real-time Payout Ledger Board</span>
//             <span className="text-zinc-300 dark:text-zinc-800">•</span>
...
export default async function ReferralIntelligenceView(props: ReferralIntelligenceViewProps) {
  ...
  const dateRangePayload = { startDate: from, endDate: to };

  // ✅ FIX 1: Pass dateRangePayload to ALL actions so whole view syncs with Date Range Picker
  const [
    performanceResult,
    executiveResult,
    funnelResult,
    ...
    getReferralPerformance(dateRangePayload),
    getLoyaltyExecutiveSummary(dateRangePayload),
    getLoyaltyFunnel(dateRangePayload),
    getLoyaltyGoals(),
    getReferralTransactionLogs(Number(searchParams?.page) || 1),
