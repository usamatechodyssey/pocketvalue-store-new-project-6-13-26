...
// =========================================================================
// 🔥 CACHED SEARCH RESULTS (Edge Caching with Safe Revalidation TTL)
// =========================================================================
const getCachedSearchResults = async (
  query: string,
  slugs: string[],
  ...
  const cacheKey = `search_results_v3:${safeQuery}:${slugs.sort().join("-")}:${sort}:${page}`;
  ...
  return unstable_cache(
    async () => {
      ...
      // Case B: Regular Text Search or Catalog Exploration
      const productData = await getPayloadProducts({
        searchTerm: safeQuery || undefined,
        sortOrder: sort,
        page: page,
      });
      ...
      return {
        ...
        filterData: productData.filterData,
        title: safeQuery ? `Results for "${safeQuery}"` : "Explore Products",
      };
    },
    [cacheKey],
    {
      tags: [`search-${safeQuery || "empty"}`],
      revalidate: 60, // ✅ FIX 1: 60s TTL prevents stale empty cache lockups on empty search queries
    }
  )();
};
      ...
      <div className="max-w-480 mx-auto">
        ...
        {products.length > 0 ? (
          <ProductListingClient
            ...
            totalCount={totalCount}
            // ✅ FIX 2: Passed 'sort: sort' in context object so client-side state matches server-side sort order!
            context={{ type: "search", value: q, sort: sort }}
            lowStockThreshold={lowStockThreshold}
          />
        ) : (
          <div className="text-center py-24 bg-white dark:bg-gray-900 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-800">
